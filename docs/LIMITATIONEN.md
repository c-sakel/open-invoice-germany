...
- **Online-Annahme durch den Kunden** ist umgesetzt (Phase 3b): ein Angebotslink (`/angebot/<token>`, ohne Login) zeigt Angebot + PDF und erlaubt Annehmen/Ablehnen (Name Pflicht, E-Mail/Kommentar optional). Bekannte Einschränkungen:
  ...
  - **Der Klartext-Token ist nur einmal sichtbar** (bei der Erzeugung, im Betreiber-Dialog). Er wird nie in der Datenbank gespeichert (nur `SHA-256`-Hash) — ein verlorener Link kann nicht wiederhergestellt werden, nur ein neuer erzeugt werden. Aus demselben Grund kann `{{offer.link}}` in E-Mail-Vorlagen nur die URL eines gerade neu erzeugten Links einsetzen; existiert für ein Angebot bereits ein gültiger Link, bleibt der Platzhalter leer (die URL muss dann manuell aus dem Link-Panel auf der Dokumentseite kopiert werden).
