...
- **Zahlungsdialog** (`#zahlung`, nur Rechnungen): „Zahlung erfassen"/„Als bezahlt markieren" öffnen denselben Dialog — vorbelegt mit dem offenen Betrag, dem heutigen Datum und der Standardzahlungsmethode. Gebucht wird **nie** automatisch, erst mit Klick auf „Buchen" im Formular. Ein Link auf `#zahlung` (z. B. aus dem Mahnwesen oder einer Liste) öffnet den Dialog direkt beim Laden der Seite; „Abbrechen"/Escape schließen ihn wieder, ein erneuter Klick auf denselben Link öffnet ihn erneut.
- **Breit/Schmal** (PDF-Ansicht, alle drei Belegarten): der Umschalter neben dem eingebetteten PDF vergrößert den Rahmen auf nahezu Fensterhöhe (Standard: A4-Seitenverhältnis). Das ist eine reine Anzeigeeinstellung **in diesem Browser** (`localStorage`) — nicht am Beleg gespeichert und nicht geräteübergreifend; ein anderes Gerät oder ein privates Fenster zeigt wieder die Standardbreite. Seitenzahl, Blättern, Zoom, Suche und Drucken liefert dabei vollständig der eingebaute PDF-Betrachter des Browsers, siehe [LIMITATIONEN.md](LIMITATIONEN.md).
