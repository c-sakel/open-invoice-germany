...
- **Token-Hashing**: `src/domain/quote-share/token.ts` erzeugt ein 256-Bit-Zufallstoken (`generateToken`); gespeichert wird ausschließlich `SHA-256(token)` (`QuoteShareLink.tokenHash`, `@unique`). Der Klartext verlässt die Domain-Funktion `createShareLink` **nur einmal**, in der Antwort — weder die Datenbank noch spätere Aufrufe können ihn zurückliefern. Das betrifft auch `{{offer.link}}` in E-Mail-Vorlagen (`src/domain/email/compose.ts`, `resolveOfferLink`): existiert für ein Angebot bereits ein gültiger Link, kann dessen URL **nicht** angezeigt werden — der Platzhalter erzeugt nur dann automatisch einen neuen Link, wenn noch keiner gültig ist, sonst bleibt er leer (siehe `docs/LIMITATIONEN.md`).
...
- **Betreiber-Sicht**: `ShareLinkPanel`/`ShareLinkPanelClient` (`src/components/`) auf `dokumente/[id]` (nur `kind=ANGEBOT`, Status DRAFT/SENT/EXPIRED) erzeugen/widerrufen Links über `/api/documents/[id]/share-links(/[linkId])` und zeigen den Klartext-Link genau einmal in einem Dialog mit Kopieren-Button.
