...
- `snapshot.ts` (`ensureDunningSnapshots`) — Selbstheilung für Altmahnungen (vor Phase 6, `snapshotSource = null`): trägt Käufer-/Verkäufer-Snapshot mit Herkunft `MIGRATION` nach; `claimBaseCents` bleibt `0` (nicht rekonstruierbar, siehe `docs/LIMITATIONEN.md`), das PDF fällt für diese Fälle auf live berechnete `payableBaseCents` zurück.
