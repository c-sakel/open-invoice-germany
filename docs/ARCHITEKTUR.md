...
**Abgeleiteter Abrechnungsstand als Bulk-Index** (`billingStateIndex`, `deriveBillingState`, `src/domain/document/billing-state.ts`, Task 5): `deriveBillingState(facts)` ist die EINE Entscheidungskette (FULL/PARTIAL/NONE aus CONVERTED_TO/PARTIAL_OF/DOWNPAYMENT_OF/FINAL_FOR-Relationen plus Mengendeckung), von `billingStateFor` (ein Angebot, Detailseite) UND `billingStateIndex` (alle betroffenen Angebote einer Organisation, Listenseite) gemeinsam genutzt — keine zweite, schleichend abweichende Kopie. `billingStateIndex` ist mit React `cache()` je Request memoisiert; fünf Bulk-Abfragen statt einer je Angebot. Seit Task 8 (Review-Finding) läuft die dritte Bulk-Abfrage (Angebots-Bruttosummen) **nicht** mehr über alle Angebote der Organisation, sondern nur über die tatsächlich verknüpften (Vereinigung der Relations-Maps) — ein Angebot ganz ohne Rechnungsbezug ist immer `NONE` und braucht weder seine Bruttosumme noch einen eigenen Map-Eintrag. **Wichtig für Aufrufer:** `states.get(id)` liefert `undefined` für ein unverknüpftes Angebot — per Vertrag gleichbedeutend mit `"NONE"` (`states.get(id) ?? "NONE"`), nie ein bares `.get(id)`. `available: false` ab `BILLING_INDEX_RELATION_LIMIT` (20.000) Relationszeilen je Organisation — dann bleibt `states` leer, Aufrufer blenden abgeleitete Zähler/Tabs/Chips aus statt zu raten.
