...
`pdfkit` 0.19 unterstützt PDF/A nativ (`subset: "PDF/A-3b"` erzeugt sRGB-`OutputIntent`, `pdfaid:part`/`conformance` in der XMP, `/AF`+`/AFRelationship` über `doc.file(...)`); die einzige verbleibende Hürde war die Schrifteinbettung — pdfkits eingebaute Helvetica ist eine AFM-Referenz ohne eingebettetes Font-Programm (verletzt ISO 19005-3, Klausel 6.2.11.4.1). Die Machbarkeit wurde vorab am 2026-09-12 mit veraPDF 1.30.2 geprüft (Spec R10): mit eingebetteter Schrift **146 Regeln bestanden, 0 verletzt**; ohne eingebettete Schrift schlägt exakt diese eine Klausel fehl. Der verbindliche Nachweis für den tatsächlichen Produktionscode läuft ab Task 7 automatisiert in CI (`scripts/render-pdfa-samples.ts` + `verapdf`) sowie einmalig lokal — siehe dortiger Abschnitt.
