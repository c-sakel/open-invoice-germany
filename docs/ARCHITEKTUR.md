...
- `BrandingSettings` (neu, `src/domain/settings/branding.ts`) — Logo (`logoPath`, `logoWidthMm`, Phase 12a: 10–140 mm), Hintergrund (`backgroundPath`, `showBackground`), Primärfarbe, vier Ränder (mm), Schriftgröße, Absenderzeile, dreispaltige Fußzeile. **Ein Satz je Organisation** (kein Layout je Belegtyp/Kunde, siehe `docs/LIMITATIONEN.md`). Die Logo-**Höhe** ist unabhängig von `logoWidthMm` fest auf `LOGO_MAX_HEIGHT_MM = 35` mm begrenzt (`src/lib/pdf/layout.ts`) — alle drei Zeichenorte (`layout.ts#drawLogo`, `layouts/modern.ts`, `layouts/klassik.ts`) übergeben pdfkit `fit: [Breite, LOGO_MAX_HEIGHT_MM]`, ein breites/flaches Logo skaliert also bis zur eingestellten Breite, ein hohes/schmales Logo wird schon vorher durch die Höhengrenze proportional verkleinert.
