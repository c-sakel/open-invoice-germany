...
**Einklappbare Abschnitte** (`CollapsibleSection`, `src/components/detail/CollapsibleSection.tsx`): ebenfalls `<details>`-basiert, mit `summary`-Zusatztext (z. B. „12 Positionen · Netto 1.234,00 €") und optionalem `defaultOpen`. Trägt auf allen drei Detailseiten den Positionsblock; bei Rechnungen zusätzlich „Zahlung erfassen" (`defaultOpen={canPay}`, in `InvoiceStatusCard`). Die interne Notiz (`InternalNotesBox`, `src/components/detail/InternalNotesBox.tsx`) steht seit der Fix-Welle (I3) davor, außerhalb der Sektion, immer sichtbar — sie steht als einziges Beleg-Feld nirgends sonst (nie im PDF/XRechnung/ZUGFeRD/Mail).
