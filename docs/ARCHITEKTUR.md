...
**PDF-Ansicht** (`PdfStack`, `src/components/detail/PdfStack.tsx`): `<iframe src="…#toolbar=0&navpanes=0">` im A4-Seitenverhältnis (CSS `aspect-ratio: 1 / 1.4142`), darunter ein Download-Link „PDF öffnen" als Fallback für Browser ohne eingebetteten PDF-Viewer. Ohne `src` (Lieferschein-Entwurf ohne Nummer, noch kein PDF) zeigt `PdfStack` stattdessen einen gestrichelten Platzhalter mit Hinweistext (`emptyText`-Prop).

**Statuskarte** (`StatusCard`, `src/components/detail/StatusCard.tsx`): generische Karte mit Titelzeile (`status`-ReactNode rechts daneben, i. d. R. `StatusBadge`), einer `dt`/`dd`-Zeilenliste (`StatusRow[]`) sowie `children` darunter für interaktive Blöcke. Je Belegtyp gibt es eine eigene, typisierte Hülle: `InvoiceStatusCard`/`DocumentStatusCard`/`DeliveryNoteStatusCard` (`src/app/{rechnungen,dokumente,lieferscheine}/[id]/_parts/`) bündeln die vormals separaten „Empfänger"/„Eckdaten"-Karten als kompakte Zeilen (inkl. Kundenanschrift, bei Rechnungen zusätzlich `USt-IdNr.`, Fix-Welle M2) und reichen den jeweils wirksamen Status als `<StatusBadge status={…} />` durch — bei Rechnungen `invoice.status` (bereits der wirksame Status aus `effectiveInvoiceStatus`, siehe `invoice-view-model.ts`), bei Dokumenten `effectiveQuoteStatus(...)` (page.tsx), bei Lieferscheinen `dn.status`. `InvoiceStatusCard` bettet zusätzlich `PaymentForm`/Zahlungsliste/Mahnblock ein (`id="zahlung"`-Anker für den `#zahlung`-Primäraktions-Link im Kopf); `DocumentStatusCard` nimmt zusätzlich `ShareLinkPanel`/`AttachmentPanel`/`DocumentChain` als `children` entgegen, `DeliveryNoteStatusCard` nur `AttachmentPanel`/`DocumentChain` (Lieferscheine haben keinen öffentlichen Angebotslink).
