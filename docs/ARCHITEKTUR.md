...
2. **`logActivity()` wirft nie** — ein Fehler beim Schreiben des Log-Eintrags (z. B. DB-Kontention) darf das eigentliche Geschäftsereignis (Rechnung festschreiben, Zahlung buchen, …) **nicht** verhindern; Fehler werden nur geloggt, nie propagiert. `ChangeLog`-Einträge bleiben dagegen **im selben Commit** wie das Ereignis Pflicht (GoBD-Unveränderbarkeit, siehe Abschnitt „GoBD-Unveränderbarkeit" oben) — ein Fehlschlag dort muss die Transaktion abbrechen.
