...
**Blöcke** (`src/components/editor/blocks/`, seit Phase 13b einspaltig untereinander statt zweispaltig): `EditorHeader` (Titel, Zurück mit Ungespeichert-Bestätigung, Speichern-Button; die Bestätigung fängt bei ungespeicherten Änderungen auch Klicks auf App-Shell-Links wie Sidebar/Topbar ab, nicht nur den eigenen Zurück-Link — die Befehlspalette (`CommandPalette`) navigiert dagegen per `router.push`, nicht über `<a href>`, und wird deshalb über einen eigenen `unsavedRef`/`setUnsaved`-Kontext in `ShellProvider` erfasst, siehe unten), `RecipientBlock` (Kundensuche über `CustomerPicker`, Ansprechpartner-/Adress-Selects, `TakeOverPrompt`-Einbindung), `MetaBlock` (Datum/Zahlungsmethode je nach Modus — keine Belegnummer, die entsteht erst beim Festschreiben; bei INVOICE zusätzlich Rechnungsdatum, ein an das Rechnungsdatum koppelbares Leistungsdatum sowie „Fällig am" als Datum+Tageszahl-Paar über `dueDaysFrom`/`dueDateFromDays`, `src/lib/editor/draft.ts` — reine, UTC-rechnende Funktionen ohne eigenen State: die Tageszahl wird bei jedem Render aus `issueDate`/`dueDate` neu abgeleitet, gespeichert wird weiterhin nur `dueDate`; bei INVOICE/DOCUMENT zusätzlich das Feld „Betreff" (`draft.subject`, s. u.)), `HeadTextBlock`/`FootTextBlock` (einfache `<textarea>`, `TextTemplatePicker`-Anbindung; Rich-Text gibt es nur beim Positions-Langtext, siehe `LineItemsEditor`/`RichTextField`), `LineItemsEditor` (Positionstabelle, siehe unten), `DocumentAdjustmentFields` (Beleg-Rabatt/-Aufschlag — Prozent **und** Festbetrag je Richtung —, hinter dem eingeklappten Link „+ Gesamtrabatt"; nicht bei DELIVERY_NOTE, dasselbe BG-20/BG-21-Mapping wie zuvor, `src/lib/einvoice/mapper.ts`), `TotalsBlock` (Summen je Steuersatz, nicht bei DELIVERY_NOTE), `MoreOptions` (Hinweis/Notiz, interne Notizen, `PrintOptionsPanel`, Skonto), `AttachmentsBlock` und `PreviewSheet` (PDF-Vorschau-Dialog, siehe unten). `CustomerPicker`/`ProductPicker`/`TextTemplatePicker` erlauben Inline-Anlage über `NewCustomerDialog`/`NewProductDialog`, ohne den Editor zu verlassen — der `ProductPicker` sitzt seit Phase 13b direkt in der Positionszeile (Link „Produkt auswählen"), nicht mehr in einem separaten Suchfeld darüber.

**Anhang VOR dem ersten Speichern** (`AttachmentsBlock`/`AttachmentPanel.ensureDocId`, Phase 13b): im Neuanlage-Editor ist `AttachmentPanel`s `docId`-Prop anfangs leer; laedt der Nutzer trotzdem eine Datei hoch, ruft `AttachmentPanel.upload()` das von `DocumentEditor` uebergebene `ensureDocId` auf — `() => save({ navigate: false })`, **derselbe** Speicherweg wie der Speichern-Button, nur ohne anschliessenden `router.push`. Schlaegt die Validierung fehl, liefert `ensureDocId` `null`, der Upload bricht ab (kein Beleg, kein Anhang, keine zweite Upload-Route). Re-Entrancy-Guard (Fix-Welle nach Task 6): `DocumentEditor.save()` wrappt den eigentlichen Speichervorgang ueber `src/lib/editor/save-guard.ts#createSaveGuard` — ein zweiter `save()`-Aufruf, waehrend der erste noch laeuft (z. B. Speichern-Knopf und `ensureDocId` fast gleichzeitig), bekommt dasselbe Promise zurueck statt einen zweiten POST auszuloesen; `AttachmentPanel.upload()` blockt einen parallelen Aufruf zusaetzlich synchron ueber ein `uploadingRef` (Drag&Drop hat sonst keinen State-Schutz, da `uploading` erst im naechsten Render greift).

**Positionstabelle** (`LineItemsEditor`/`LineRow`/`LineRowMenu`/`LineDiscountField`/`UnitSelect`): Zeilenmenü „Typ ändern" wechselt zwischen Position/Überschrift/Textblock/Zwischensumme über dieselbe `setLine`-Aktion wie jede andere Feldänderung (ausgeblendet bei DELIVERY_NOTE, das keinen `lineType` kennt); ein Segmentumschalter „Preise: netto/brutto" (`aria-pressed`, seit Phase 13b Task 3 kein Kontrollkästchen mehr) ist reine Darstellung (`draft.grossDisplay`, Teil des Entwurfs) — das Preisfeld selbst bleibt immer netto gebunden, gespeichert wird ausschließlich netto. Zeilen lassen sich per Drag-and-Drop **oder** `Alt+↑`/`Alt+↓` auf einer Zeile umsortieren; `Enter` im Beschreibungs-/Bezeichnungsfeld der letzten Zeile legt eine neue Zeile an und fokussiert sie.
...
- `PrintSettings` (neu, `src/domain/settings/print.ts`) — zehn globale Druck-Schalter (Fußzeile, Seitenzahlen, Falz-/Lochmarken, Artikelnummer-/Beschreibungs-/Steuersatz-/Zeilensummen-Spalte, Absenderzeile, GiroCode) plus die numerische **GiroCode-Größe** `giroSizeMm` (Phase 12a, `src/schemas/settings.ts`, 15–40 mm, Standard 22 mm — unter 15 mm ist der EPC-QR nicht mehr zuverlässig scanbar). `effectivePrintOptions(global, overrideJson)` ist eine reine Merge-Funktion: das gespeicherte `printOptionsJson` eines einzelnen Belegs (Invoice/Quote/DeliveryNote) überschreibt gezielt einzelne globale Werte — `giroSizeMm` folgt denselben Regeln wie die zehn Schalter (Beleg-Override, `freezePrintOptionsJson` beim Festschreiben). `setPrintOptions(orgId, {kind, id}, rawInput)` prüft selbst `status === "DRAFT"` (409 sonst) — für Invoice zusätzlich hinter dem bestehenden GoBD-Guard (`src/lib/db.ts`), für Quote/DeliveryNote reicht die Domain-Prüfung, da diese Modelle keinen eigenen DB-Guard haben.
- `BrandingSettings` (neu, `src/domain/settings/branding.ts`) — Logo (`logoPath`, `logoWidthMm`, Phase 12a: 10–140 mm), Hintergrund (`backgroundPath`, `showBackground`), Primärfarbe, vier Ränder (mm), Schriftgröße, Absenderzeile, dreispaltige Fußzeile. **Ein Satz je Organisation** (kein Layout je Belegtyp/Kunde, siehe `docs/LIMITATIONEN.md`). Die Logo-**Höhe** ist unabhängig von `logoWidthMm` fest auf `LOGO_MAX_HEIGHT_MM = 35` mm begrenzt (`src/lib/pdf/layout.ts`) — `layout.ts#drawLogo` und `layouts/klassik.ts` übergeben pdfkit `fit: [Breite, LOGO_MAX_HEIGHT_MM]`, ein breites/flaches Logo skaliert also bis zur eingestellten Breite, ein hohes/schmales Logo wird schon vorher durch die Höhengrenze proportional verkleinert. **Ausnahme `layouts/modern.ts`**: dort begrenzt nicht `LOGO_MAX_HEIGHT_MM`, sondern die Höhe des Kopfbalkens (`HEADER_BAR_H = 54` pt abzüglich Rand, `fit: [Breite, barH - mm(4)]`) — das sind rund **15 mm**, deutlich weniger als in den anderen beiden Layouts. Ein breites Logo bei `logoWidthMm = 140` rendert im Layout `modern` also spürbar kleiner als in `standard`/`klassik`.
...
- `drawKopf(frame, input) => number` — Titel/Nummer/Empfänger/Meta auf Seite 1, liefert die
  Start-y-Position fuer den restlichen Seiteninhalt. Seit Phase 13b ruft jedes `drawKopf` der
  vier eigenstaendigen Layouts (`standard`/`schlicht`/`klassik`/`modern`; `blau`/`schwarz`/
  `kompakt` erben ueber `styledLayout`) vor `input.intro` `drawSubject(frame, subject, y)`
  (`layouts/shared.ts`) auf, sofern `input.subject` gesetzt ist — eine fette Zeile in voller
  Textbreite direkt ueber dem Kopftext. `drawSubject` trimmt selbst; ein leerer oder nur aus
  Leerzeichen bestehender Betreff zeichnet nichts (liefert `y` unveraendert zurueck, byte-
  gleiche PDF-Ausgabe zum Referenzfall ohne Betreff). Quelle: `Invoice.subject`/`Quote.subject`
  (Prisma, seit Phase 4b), durchgereicht ueber `EInvoiceData.subject`
  (`src/lib/einvoice/types.ts`) an `mapper.ts`/`pdf-data.ts`/`preview-draft.ts`. Dieselbe
  getrimmte Regel gilt fuer den BT-22-Betreff-Note mit Subjektcode BT-21 „AAI" in UBL
  (`xrechnung.ts`, Praefix `#AAI#`) und CII (`cii.ts`, `ram:Content` vor `ram:SubjectCode`)
  — leer/nur Leerraum erzeugt in keinem der drei Formate ein Element.
...
**Aktionsmatrix** (`availableActions`, `src/domain/document/actions.ts`): eine reine Funktion `(doc: ActionableDoc) => ActionKey[]` (`OPEN | EDIT | DUPLICATE | PDF | XRECHNUNG | SEND | RESEND | PAYMENT | REMINDER | DUNNING | DELIVERY_NOTE | CANCEL | CONVERT`) je `DocKind` (`INVOICE | QUOTE | DELIVERY_NOTE | RECURRING`) — ersetzt die zuvor auf der Rechnungs-Detailseite verstreuten Sets (`CANCELLABLE_TYPES`/`CREDITABLE_TYPES`/`NOT_DUPLICATABLE_TYPES`). Für `INVOICE` erwartet sie den **wirksamen** Status (`effectiveInvoiceStatus`-Ergebnis), für `QUOTE` den wirksamen Angebotsstatus (`effectiveQuoteStatus`, Phase 3a) — `DELIVERY_NOTE`/`RECURRING` nutzen den rohen DB-Status. Genutzt von `RowActionsMenu` (Listen) und der Rechnungs-Detailseite; die Übergangs-Zustandsmaschinen von Angebot/Lieferschein-Detailseiten (MARK_SENT/MARK_ACCEPTED/…) bleiben eigenständig, da `availableActions` diese Übergänge nicht abbildet. `CONVERT` (Phase 13a, Task 7) bündelt die AB-/Rechnungs-/Lieferschein-Erzeugung eines Angebots in einem Menüpunkt — `convertTargets(doc)` (dieselbe Datei) liefert `{orderConfirmation, invoice, deliveryNote}`, einzige Quelle für `ConvertMenu`s Sichtbarkeits-Props auf Detailseite **und** in `RowActionsMenu` — keine zweite, lokale Übergangstabelle. `orderConfirmation`/`invoice` haengen von Status UND Abrechnungsstand ab (`convertedToInvoiceId`/`billingFull`); `deliveryNote` folgt seit Fix-Welle 1 (M5) EXAKT der serverseitigen Regel in `convertToDeliveryNote` (`src/domain/document/convert.ts`) — NUR der Status, weder `kind` (PROFORMA eingeschlossen) noch der Abrechnungsstand sperren die Lieferschein-Erzeugung (nach der Rechnung noch zu liefern ist der Normalfall).

**Listenfilter/-suche** (`listInvoices`/`src/domain/invoice/list.ts`, `listQuotes`/`listDeliveryNotes`/`listRecurring`/`src/domain/document/list.ts`): je ein Zod-Filterschema (`invoiceListFilterSchema` u. a.) mit `status`/`type`/`kind`/`customerId`/`from`/`to`/`minCents`/`maxCents`/`number`/`paymentMethodId`/`eInvoice`/`currency`/`q`/`includeArchived`/`limit`/`offset`/`sort` (nicht jedes Feld in jedem Schema — `type`/`sort` nur bei Rechnungen, `kind`/`includeArchived` nur bei Angeboten/ABs bzw. Lieferscheinen), org-gescoped, DB-portabel (`ciContains()` in `src/lib/db.ts` für case-insensitive Suche, providerabhängig über den `DATABASE_URL`-Präfix — SQLite `LIKE` ist bereits case-insensitive für ASCII, Postgres nutzt `mode: "insensitive"`). Volltextsuche (`q`) deckt bei `listInvoices` `number`/`orderNumber`/`customer.name`/`lines.description` ab, bei den übrigen Listen nur `number`/`title`+`customer.name`. Konsumiert von serverseitig gerenderten Listenseiten (`Pagination`, reines `<form method="get">`, kein Client-JS; `FilterBar` ist seit Phase 13a Task 6 eine Client-Komponente mit `<form>`-Rückfall, siehe Abschnitt „Listen: Status-Tabs, Kopfkennzahlen, Herkunft, Fälligkeit & Abrechnungsstand" unten) sowie den GET-Routen `/api/invoices`, `/api/documents`, `/api/delivery-notes` (`/abos` liest `listRecurring` direkt serverseitig, keine eigene REST-Route) und dem MCP-Tool `list_invoices`.
...
**Navigation**: `MainNav` (gruppiert: Dashboard · Verkauf · Stammdaten · Einstellungen) ersetzte die vorherige flache Navigationsleiste; Gutschriften sind über `/rechnungen?type=CREDIT_NOTE` erreichbar (kein eigener Menüpunkt/keine eigene Route, der Filter existiert seit Task 1). `MainNav` selbst wurde in Phase 11a durch die einklappbare Sidebar ersetzt (siehe Abschnitt "App-Shell, Navigation & Suche (Phase 11a)" unten).
...
### Listen: Status-Tabs, Kopfkennzahlen, Herkunft, Fälligkeit & Abrechnungsstand (Phase 13a)

**Filterbedingungen ohne Status** (`invoiceFilterConditions`/`src/domain/invoice/list.ts`, `quoteFilterConditions`/`deliveryNoteFilterConditions`/`recurringFilterConditions`/`src/domain/document/list.ts`): aus den bisherigen `listInvoices`/`listQuotes`/… herausgezogen — jede Funktion baut das `Prisma...WhereInput[]`-Array AUSSER der Statusbedingung, damit Liste **und** Tab-Zähler exakt dieselbe Filtermenge sehen (ein Statusbegriff, keine zweite Kopie). `quoteFilterConditions` nimmt zusätzlich einen genuinen (nicht aus dem Zod-Schema stammenden) Parameter `opts.ids?: string[]` entgegen — das Seiten-Wiring von `/dokumente` übersetzt `?status=billed`/`?status=partially-billed` (kein gespeicherter Beleg-Status) damit in `id: { in: ids }` (Ids aus `billingStateIndex`), ohne `quoteListFilterSchema` um ein Feld zu erweitern, das sonst auch als ungewollter öffentlicher Query-Parameter der `/api/v1/Quote`-/`/api/v1/OrderConfirmation`-Routen erschiene (dieses Schema ist deren `request.query`-Validierung).

**Status-Tab-Zähler** (`invoiceStatusTabCounts`/`quoteStatusTabCounts`/`deliveryNoteStatusTabCounts`/`recurringStatusTabCounts`, dieselben Dateien): je Tab ein `count()` über denselben Basisfilter plus der jeweiligen Statusbedingung — feste Abfragezahl (ein `count()` je Tab), keine Zeilen im Speicher. Die Tabs überlappen sich bewusst dort, wo es die Statuslogik selbst tut (eine teilbezahlte, überfällige Rechnung zählt unter „Teilbezahlt" **und** unter „Überfällig"). `quoteStatusTabCounts` ergänzt zwei ABGELEITETE Tabs, die keinen Beleg-Status abbilden, sondern den Abrechnungsstand (`billingStateIndex`, s. u.) — „billed" (FULL) und „partially-billed" (PARTIAL); ohne verfügbaren Index (Obergrenze überschritten) liefert sie für beide `null`, die Seite blendet die Tabs dann aus statt eine falsche Zahl zu zeigen.

**Kopfkennzahlen** (`invoiceListHeadline`/`src/domain/invoice/list.ts`, `quoteListHeadline`/`src/domain/document/list.ts`): Anzahl + Σ Brutto (bei Rechnungen zusätzlich Offen/Überfällig) über die **gefilterte** Menge (nicht die angezeigte Seite) — `groupBy({ by: ["currency"] })` liefert Anzahl/Summe DB-seitig exakt je Währung, `mixedCurrency` (mehr als eine Gruppe) steuert den Hinweis „gemischte Währungen" in der UI, statt Beträge über Währungen hinweg stillschweigend zu addieren. Offen/Überfällig bei Rechnungen laufen über dieselbe Formel wie überall sonst (`openAmountCents` + `effectiveInvoiceStatus`, lokal auf `Math.max(0, …)` geklemmt gegen eine überzahlte Rechnung), keine eigene Geldformel. `/lieferscheine` zeigt seit Fix-Welle 1 (M4) ebenfalls eine Kopfkennzahl (`deliveryNoteListHeadline`), aber NUR die Anzahl — `DeliveryNote` speichert (kein Steuerbeleg) weder eine Bruttosumme noch eine Währung, eine aus den optionalen, meist ungesetzten Zeilenpreisen hochgerechnete Summe wäre keine belastbare Zahl. `/abos` bleibt ohne Kopfkennzahl (Spec: unverändert).

**Abgeleiteter Abrechnungsstand als Bulk-Index** (`billingStateIndex`, `deriveBillingState`, `src/domain/document/billing-state.ts`, Task 5): `deriveBillingState(facts)` ist die EINE Entscheidungskette (FULL/PARTIAL/NONE aus CONVERTED_TO/PARTIAL_OF/DOWNPAYMENT_OF/FINAL_FOR-Relationen plus Mengendeckung), von `billingStateFor` (ein Angebot, Detailseite) UND `billingStateIndex` (alle betroffenen Angebote einer Organisation, Listenseite) gemeinsam genutzt — keine zweite, schleichend abweichende Kopie. `billingStateIndex` ist mit React `cache()` je Request memoisiert; fünf Bulk-Abfragen statt einer je Angebot. Seit Task 8 (Review-Finding) läuft die dritte Bulk-Abfrage (Angebots-Bruttosummen) **nicht** mehr über alle Angebote der Organisation, sondern nur über die tatsächlich verknüpften (Vereinigung der Relations-Maps) — ein Angebot ganz ohne Rechnungsbezug ist immer `NONE` und braucht weder seine Bruttosumme noch einen eigenen Map-Eintrag. **Wichtig für Aufrufer:** `states.get(id)` liefert `undefined` für ein unverknüpftes Angebot — per Vertrag gleichbedeutend mit `"NONE"` (`states.get(id) ?? "NONE"`), nie ein bares `.get(id)`. `available: false` bei MEHR ALS `BILLING_INDEX_RELATION_LIMIT` (Fix-Welle 1, S5: 20.000 auf 5.000 gesenkt — die aus den Relationszeilen abgeleiteten Id-Listen gehen ungeteilt als `id: { in: [...] }` in eine Abfrage, 5.000 haelt sicheren Abstand zur SQLite-Bind-Parameter-Grenze) Belegverknüpfungen (Relationszeilen, nicht Belegen — ein Beleg kann mehrere tragen) je Organisation — dann bleibt `states` leer, Aufrufer blenden abgeleitete Zähler/Tabs/Chips aus statt zu raten; ein gemerkter Link wie `/dokumente?status=billed` liefert in diesem Fall still **0 Treffer** (`id: { in: [] }`), nicht nur ausgeblendete Tabs.

**Herkunft je Zeile** (`originsFor`, `src/domain/document/origin.ts`, Task 2): liefert je Beleg-Id einer ganzen Listenseite die Herkunft (Link + Label) — zwei Quellen in fester Reihenfolge, die denormalisierten Felder am Beleg (`Invoice.sourceType/sourceId`/`.recurringInvoiceId`, `DeliveryNote.sourceType/sourceId`) und, nur für Belege ohne solche, die `CONVERTED_TO`-Relation. Feste Abfragezahl (zwei plus maximal vier Quellabfragen je Seite), kein zweiter Relationsbegriff neben `src/domain/relations.ts`.

**Relative Fälligkeit** (`relativeDueLabel`, `src/lib/relative-date.ts`, Task 2): reine Tagesdifferenz („in 5 Tagen"/„seit 2 Tagen überfällig") statt `Intl.RelativeTimeFormat` (das ab rund einer Woche auf Wochen/Monate umschaltet — für ein Zahlungsziel unbrauchbar), Tagesgrenze über `utcDateOnly` — dieselbe UTC-Konvention wie `effectiveInvoiceStatus`/`listInvoices`/Mahnwesen.

**UI-Bausteine** (`src/components/list/`): `StatusTabs` (Links auf `?status=…`, `offset` fällt beim Tabwechsel weg), `ListHeadline` (reine Anzeige der Kopfkennzahlen-Kacheln), `FilterBar` (Task 6, `"use client"`) navigiert bei jeder Änderung selbst — Selects/Datumsfelder sofort, Text-/Betrags-/Kontaktfelder mit einem EIGENEN 300-ms-Timer je Feld (Fix-Welle 1, M1 — ein einzelner geteilter Timer verwarf zuvor den anstehenden Wert eines zweiten, kurz danach editierten Feldes), kein sichtbarer „Filtern"-Knopf mehr. Betrags- (`minCents`/`maxCents`) und Kontaktfelder (`customerId`) bilden ihren Rohtext seit Fix-Welle 1 (M2) NICHT mehr selbst auf Cent/Id ab — derselbe Rohtext geht mit UND ohne JavaScript in die URL, serverseitig übernehmen `parseListQuery`s `moneyKeys` (tolerant Euro → Cent) bzw. `applyCustomerComboFilter` (`src/domain/customer/list.ts`, exakter Treffer → Id, sonst Rückfall auf `q`) dieselbe Abbildung für beide Pfade — der JS-freie Rückfall (`<form method="get">`, Enter/Absenden) liefert seither dieselben Ergebnisse wie der JS-Pfad, nicht nur „ohne Entprellung". `RowActionsMenu` bekommt seit Task 7/8 zusätzlich `convert`/`documentActions`-Props (bündelt `ConvertMenu` bzw. `DocumentActionsMenuItems` ins Zeilenmenü) sowie Direktknöpfe (PDF/Zahlung) vor dem „⋯"-Menü; `RowPaymentDialog` kapselt den Zahlungs-Dialog als eigenes `<dialog>`. `PageContainer` (Task 1, `src/components/PageContainer.tsx`) gibt Listen-/Editor-/Belegseiten die volle `AppShell`-Breite (1600 px) frei und schnürt nur Formular-/Einstellungsseiten (`width="form"`, über `src/app/einstellungen/layout.tsx` und einzelne Formularseiten) auf eine lesbare Zeilenlänge ein.

### Grafiken & Auswertungen (Phase 12e)

**Umsatz je Monat und Top-Kunden** (`monthlyRevenue`/`netShareCents`/`signedRevenueShareCents`/`monthKey`, `src/domain/reporting/revenue.ts`; `topCustomers`, `src/domain/reporting/customers.ts`): reine, org-gescopte Aggregationsfunktionen über `select`-reduzierte `Invoice`-Zeilen + JS-Summierung (dieselbe DB-Portabilitäts-Begründung wie `dashboardSummary`, siehe Abschnitt „Workflow & UX … Dashboard" oben). Basis ist **Netto**, nicht Brutto — `netShareCents(inv)` liefert ohne `payableCents` schlicht `netTotalCents`; trägt die Rechnung `payableCents` (Schlussrechnung einer Abschlagskette, §14), skaliert sie `netTotalCents` proportional zum Verhältnis `payableCents / grossTotalCents` (`roundHalfUp`). Ohne diese Skalierung würde eine Abschlags- plus Schlussrechnung den Abschlag doppelt zählen — derselbe Fehler, den die Fix-Welle S2 ursprünglich für `revenueThisMonthCents`/`grossTotalCents` in `dashboardSummary` behoben hat (Fix I3, Abschluss-Review: `dashboardSummary`/`customerOverview` nutzen inzwischen selbst `monthlyRevenue`/`signedRevenueShareCents` — netto, dieselbe Grundlage wie hier, kein zweites Bemessungsverfahren mehr, siehe Abschnitt „Workflow & UX … Dashboard/Kundendetailseite" oben). `monthlyRevenue` liefert eine lückenlose 12-Monats-Reihe (Default `months`, überschreibbar, optionaler `customerId`-Filter), `topCustomers` gruppiert dieselbe Beleg-Menge je `customerId`, sortiert absteigend nach Netto-Umsatz und kappt bei `limit` (Default 5; die Validierung ungültiger Werte übernimmt die Boundary — API-Route/MCP-Tool, Task 5 —, nicht die Domainfunktion selbst).

**Vorzeichen und Storno — periodengerechte (accrual) Sicht** (Fix-Welle 1 + Fix I2, Abschluss-Review): beide Funktionen summieren `signedRevenueShareCents` — `netShareCents` PLUS Vorzeichen-Normalisierung bei `type === "CREDIT_NOTE"` (`-Math.abs(...)`, nicht `-1`-Multiplikation). Gutschriften/Stornos, die über die bestehenden Domain-Funktionen entstehen (`createPartialCreditNote`, `src/domain/invoice/credit.ts`; `cancelInvoice`, `src/domain/invoice/cancel.ts`), tragen bereits **negative** `netTotalCents`/`grossTotalCents` (betragsspiegelbildlich zum Original) — `-Math.abs` ist dafür idempotent (ändert nichts). Eine FREISTEHENDE Gutschrift (Editor, `type: CREDIT_NOTE` direkt angelegt, `createInvoiceSchema` erzwingt kein Vorzeichen auf `unitNetPriceCents`) trägt dagegen typischerweise **positive** Beträge — die alte, ungedrehte Summierung (Fix-Welle 1) hätte diese Gutschrift zum Umsatz addiert statt ihn zu mindern (Review-Befund I2); eine naive `type === "CREDIT_NOTE" ? -1 : 1`-Drehung hätte umgekehrt eine echte Domain-Gutschrift ein zweites Mal negiert. `-Math.abs` behandelt beide Fälle korrekt. Der Statusfilter schließt nur `status === "DRAFT"` aus, **nicht** `CANCELLED`: das stornierte Original bleibt unverändert (GoBD-Unveränderbarkeit) und zählt weiterhin mit vollem Betrag in seinem Ausstellungsmonat; die Storno-Gutschrift selbst trägt ihr eigenes `issueDate` (Stornozeitpunkt) und mindert dort separat. Fallen Original und Storno in unterschiedliche Monate, zeigt die Reihe also `+X` im Ausstellungs- und `-X` im Stornierungsmonat — über den vollen Zeitraum gleicht sich das exakt aus (Summe 0), statt eine einzelne Periode fälschlich negativ auszuweisen (Test: `test/integration/reporting-revenue.test.ts`, Storno-Szenario Februar/März sowie freistehende-Gutschrift-Szenario, Fix I2).

**Statusverteilung und Zahlungsverhalten** (`statusCounts`, `src/domain/reporting/status.ts`; `paymentBehaviour`, `src/domain/reporting/payment-behaviour.ts`, Task 3, §54): beide sind reine, org-gescopte Funktionen über `select`-reduzierte Zeilen, keine DB-spezifischen Aggregate (dieselbe Portabilitäts-Begründung wie oben). `statusCounts` ergänzt `dashboardSummary` nur um die dort fehlende PAID-Zahl fürs Ringdiagramm — die Kennzahlkacheln selbst bleiben bei `dashboardSummary`, kein Doppelbau (§1.4). `paymentBehaviour` liefert `avgDaysToPay` (Ø Tage Rechnungsdatum bis letzte erfasste Zahlung) und `onTimeShare` (Anteil bis zum Fälligkeitstag beglichener Rechnungen, 0..1, „am Fälligkeitstag" zählt als pünktlich) über alle `status === "PAID"`-Rechnungen; beide sind `null` ohne auswertbare Datengrundlage statt `NaN`. Rechnungen ohne Zahlungszeile (manueller Statuswechsel auf `PAID` ohne `recordPayment`) werden übersprungen — es gibt keinen Zahlungszeitpunkt, über den gemittelt werden könnte (siehe `docs/LIMITATIONEN.md`). `paymentBehaviour` liefert bewusst **kein** `openCents` — den offenen Betrag kennt `customerOverview`/`dashboardSummary` bereits.

**Gemeinsamer Kern für REST und MCP** (`runReport`, `reportQuerySchema`, `src/domain/reporting/query.ts`, Task 5): schaltet auf Basis von `type` (`revenue` \| `top-customers` \| `status` \| `payment-behaviour`) zwischen den vier Funktionen oben um und liefert einheitlich `{ objectName: "Report", type, rows }` — `payment-behaviour` verpackt sein einzelnes Ergebnis als Ein-Elemente-`rows`-Array, damit die Form über alle vier Typen stabil bleibt. `GET /api/v1/Report` (`src/app/api/v1/Report/route.ts`, Scope `read`) und das MCP-Tool `get_report` (`src/mcp/tools/system.ts`) rufen beide **ausschließlich** `runReport` auf — `reportQuerySchema` ist das einzige Validierungsschema für `months`/`limit`/`customerId` (1–36 bzw. 1–50), es gibt kein zweites, drittes Schema je Schnittstelle. `Report` ist keine CRUD-Ressource (keine Tabelle, kein `RESOURCE_SCHEMAS`-Eintrag in `src/api/openapi.ts` nötig) — die Antwort bleibt `apiDataResponseSchema(z.unknown())`, wie bei den Aktions-Endpunkten.

**Diagramm-Baukasten** (`src/components/charts/{BarChart,LineChart,DonutChart,ChartFrame}.tsx` + `types.ts`): reine, framework-typische Server-Komponenten (kein `"use client"`) — Inline-SVG ohne Chart-Bibliothek, `viewBox`/`preserveAspectRatio` für Responsivität, `role="img"` + `aria-label` + `<title>` je Segment + eine `sr-only`-Wertetabelle unter jedem Diagramm (Barrierefreiheit ohne zweite Datenquelle). `src/components/dashboard/chart-data.ts` übersetzt die Rückgaben der Reporting-Domain (`MonthlyRevenuePoint[]`, `TopCustomer[]`, `StatusCount[]`) sowie `AgingBucket[]` (`dashboard/summary.ts`) in das generische `ChartDatum[]` des Baukastens — als reine Funktionen ohne React-Rendering separat testbar (`test/unit/dashboard-charts.test.tsx`). Konsumenten: `BarChart` (senkrecht) für die Umsatzreihe und (waagerecht) für Top-5-Kunden sowie in `AgingChart.tsx`, `DonutChart` für die Statusverteilung — alle auf dem Dashboard (`DashboardWidgets.tsx`, `src/app/page.tsx`); `LineChart` für die Umsatzreihe auf der Kundendetailseite (`src/app/kunden/[id]/page.tsx`).

**Aggregation in JS, nicht in der DB** — durchgängiges Prinzip der gesamten Phase 12e-Reporting-Domain (wie bereits bei `dashboardSummary`): jede Funktion liest `select`-reduzierte Zeilen und summiert/gruppiert in JavaScript, statt datenbankspezifische Aggregatfunktionen (`GROUP BY`, Fenster-/Datumsfunktionen) zu nutzen — Portabilität zwischen SQLite (Solo) und PostgreSQL (Docker) bleibt dadurch ohne Sonderfälle je Treiber erhalten.
...
**Sidebar-Gruppen ein-/ausklappbar** (Phase 11d, Task 5; `SidebarGroup.tsx`): jede Gruppe merkt sich ihren Auf-/Zu-Zustand in `localStorage["oig.nav.open.<Gruppenschlüssel>"]` (Default offen, auch ohne Storage — privater Modus). Wechselt die Route in eine andere, bisher zugeklappte Gruppe hinein, öffnet sie sich **einmalig** beim Übergang (`activeGroupKey`-Vergleich gegen den vorherigen Wert per `useRef`) — ein erneutes Laden derselben bereits aktiven Seite pinnt die Gruppe nicht dauerhaft offen, das Zuklappen der eigenen aktiven Gruppe bleibt also wirksam. Die eingeklappte (Icon-only) Sidebar zeigt alle Gruppen immer offen, ohne Header/Chevron.

**Fokusfalle + Scroll-Sperre** (Phase 11d, Task 5; `src/lib/focus.ts#getFocusable`): ein gemeinsamer Selektor für fokussierbare Elemente, den `CommandPalette.tsx`, `Topbar.tsx` (mobiler Drawer) und `PreviewSheet.tsx` (Editor-Vorschau, Phase 11c) je für ihre eigene Tab-Zirkulation nutzen, statt drei parallel gepflegter Kopien. Beide Overlays sperren `document.body.style.overflow` während sie offen sind (Cleanup stellt den vorherigen Wert wieder her). Escape schließt jeweils nur die **oberste** Ebene: die Palette stoppt `stopPropagation`, wenn sie über dem Drawer geöffnet wurde, der Drawer reagiert nur, wenn die Palette nicht gleichzeitig offen ist (zweite Verteidigungslinie, Task-5-Fix-1) — ein Escape im mobilen Drawer mit geöffneter Palette schließt also nur die Palette, nicht beide Overlays auf einmal.

### Belegansicht (Phase 11d)

Rechnungs-, Dokument- (Angebot/AB/Proforma) und Lieferschein-Detailseiten (`src/app/{rechnungen,dokumente,lieferscheine}/[id]/page.tsx`) teilen sich seit Phase 11d einen gemeinsamen Rahmen statt drei unabhängig gewachsener Seiten — PDF mittig, Statuskarte(n) rechts, Aktionen oben (dem Vorbild moderner Rechnungsdienste, Betreiber-Auftrag Phase 11). Der frühere dedizierte `PdfPreview`-Baustein ist damit vollständig entfallen, ersetzt durch `PdfStack` (siehe unten). Phase 13c (Task 2/5) verbreitert das Raster und teilt die Statuskarte je Belegtyp in zwei getrennte Karten.

**Rahmen** (`DocumentDetailLayout`, `src/components/detail/DocumentDetailLayout.tsx`): reine Layout-Komponente ohne eigene Logik, nimmt neun benannte Slots — `nav` (siehe `DetailNav`), `title`, `badges`, `actions`, `more` (siehe `ActionMenu`), `notice`, `pdf` (Seitenmitte), `aside` (rechte Spalte) und `children` (volle Breite darunter: Positionen, Mahnblock, Korrekturbereich, E-Mail-Verlauf, Zeitstrahl). Grid `xl:grid-cols-[minmax(0,1fr)_24rem]` (Phase 13c, Task 2: breiter als zuvor `lg:…_20rem`, erst ab `xl` nebeneinander statt schon ab `lg`) — PDF und die rechte Spalte stehen darüber nebeneinander, darunter stapeln sie sich. `nav` sitzt seit Task 2 nicht mehr über dem Titel, sondern selbst in einer `DetailCard title="Beleg"` als erstem Element der rechten Spalte (Vor/Zurück näher an den übrigen Belegdaten); die Karte zeigt seit Fix-Welle 1 (N10) nur noch die Navigation, nicht mehr zusätzlich den Belegtitel (der steht bereits als `<h1>` im Kopf direkt darüber).

**Kartenrahmen der rechten Spalte** (`DetailCard`, `src/components/detail/DetailCard.tsx`, Phase 13c): dasselbe Äußere wie `StatusCard` (Titelzeile, `rounded-lg border`), aber ohne Zeilenliste — reiner Titel + `children`. Fix-Welle 1 (M1, Abschluss-Review): bis dahin hüllten die Detailseiten auch Anhänge/Dokumentenkette/Annahme-Link zusätzlich in eine `DetailCard` — die umschlossenen Panels (`AttachmentPanel`, `DocumentChain`, `ShareLinkPanelClient`) bringen aber bereits praktisch deckungsgleich dieselbe Kartenoptik (Rahmen + Überschrift) selbst mit, sodass jede dieser Karten doppelt gerahmt war und ihren Titel doppelt zeigte (z. B. „Anhänge" innen und außen). `DetailCard` wird seither nur noch für die Belegkarte („Beleg", `DocumentDetailLayout`) verwendet; die drei genannten Panels reicht jede `page.tsx` unverändert und ungehüllt direkt an den `aside`-Slot weiter. `PrintOptionsPanel` (Lieferschein-Entwurf) sitzt weiterhin ungehüllt im `children`-Slot unter `id="druckoptionen"`, nie in einer `DetailCard`.

**Navigation** (`DetailNav`, `src/components/detail/DetailNav.tsx`, Client-Komponente): Zurück-Link zur Liste plus Vor-/Zurück-Pfeile zum Nachbarbeleg **derselben gefilterten Liste**, dazu Tastatur `Alt+←`/`Alt+→` (ignoriert, wenn der Fokus in einem Eingabefeld liegt; überlagert unter Windows/Linux den Browser-Zurück/Vor-Shortcut, siehe `docs/LIMITATIONEN.md`). Pfeile ohne Nachbarn tragen `aria-disabled` und einen No-Op-`onClick`; `href="#"` bleibt als Platzhalter stehen, unterdrückt wird nur der Sprung (`preventDefault` + `pointer-events-none`).

**Listen-Nachbarschaft** (`src/domain/document/neighbors.ts`): Listen hängen ihren aktuellen Filter als `?liste=<query>` an jeden Zeilen-Link (Name `liste`, weil `from` als Datumsfilter der Listen bereits belegt ist). Die Detailseite parst das mit `listeQuerySchema` (Zod, `^[A-Za-z0-9=&%._+*-]*$` — `*` bewusst erlaubt, da `URLSearchParams.toString()`/`encodeURIComponent` es unescaped durchreichen, z. B. aus einer Freitextsuche `Foo*`), filtert auf eine feste `ALLOWED_KEYS`-Whitelist (`q, status, type, kind, from, to, offset, archiviert, customerId`) und lädt anschließend **ohne Paginierung** bis zu `NEIGHBOR_LIMIT = 200` Zeilen derselben, org-gescopten Listenfunktion (`listInvoices`/`listQuotes`/`listDeliveryNotes`), um darin den Vorgänger/Nachfolger der aktuellen Id zu finden (`neighborIds`, reiner Array-Index-Vergleich). Ein ungültiger `liste`-Wert liefert `{prevId: null, nextId: null, backQuery: ""}` statt eines Fehlers. Der Zurück-Link ist immer `/<Listenbasis>?<bereinigte Query>` (kein Open-Redirect möglich, da nur die Whitelist-Schlüssel übernommen werden); fremde Organisationen sehen keine Nachbarn, weil die zugrunde liegenden Listenfunktionen bereits `orgId`-gescoped sind.

**PDF-Ansicht** (`PdfStack`, `src/components/detail/PdfStack.tsx` + `PdfViewToolbar`, `src/components/detail/PdfViewToolbar.tsx`, Phase 13c, Task 1, Ruling „Kein PDF.js"): kein eigener PDF-Betrachter — der `<iframe src="…#view=FitH">` trägt seit Task 1 **keine** `#toolbar=0&navpanes=0`-Parameter mehr, die eingebaute Werkzeugleiste des Browser-PDF-Viewers (Seitenzahl, Blättern, Zoom, Suche, Drucken) ist also wieder sichtbar und bedienbar. `PdfViewToolbar` ergänzt nur, was der Browser-Viewer nicht bietet: „In neuem Tab öffnen" und „Herunterladen", der Rahmen selbst hat ein festes A4-Seitenverhältnis (`aspect-ratio: 1 / 1.4142`). Fix-Welle 1 (S3): ein früherer Breit/Schmal-Umschalter ist wieder entfernt — er änderte in Wahrheit nie die Breite (die Vorschau-Spalte ist im Grid bereits `minmax(0,1fr)` und damit in jedem Zustand so breit wie möglich), nur die Höhe; sie „wahr" zu machen (Vorschau-Spalte wächst, die 24rem-Spalte bleibt gleich breit) hätte einen Ausbruch aus dem app-weiten `max-w-[1600px]`-Rahmen (`AppShell`, gilt auf jeder Seite) erfordert — kein schlanker Fix in dieser Komponente, daher lieber entfernt als eine Attrappe behalten (Koordinator-Ruling). Darunter zusätzlich ein Textlink „PDF öffnen" als Fallback für Browser ohne eingebetteten PDF-Viewer (iOS/Safari, WebViews). Ohne `src` (Lieferschein-Entwurf ohne Nummer, noch kein PDF) zeigt `PdfStack` stattdessen einen gestrichelten Platzhalter mit Hinweistext (`emptyText`-Prop).

**Statuskarten** (`StatusCard`, `src/components/detail/StatusCard.tsx`): generische Karte mit Titelzeile (`title`-Prop, Default „Status"; `status`-ReactNode rechts daneben, i. d. R. `StatusBadge`, oder `null`), einer `dt`/`dd`-Zeilenliste (`StatusRow[]`) sowie optionalem `children` darunter für interaktive Blöcke. Je Belegtyp gibt es eine eigene, typisierte Hülle — `InvoiceStatusCard`/`DocumentStatusCard`/`DeliveryNoteStatusCard` (`src/app/{rechnungen,dokumente,lieferscheine}/[id]/_parts/`) — die seit Phase 13c (Task 2 für Rechnungen, Task 5 für Dokument/Lieferschein) **zwei** `StatusCard`s statt einer rendert: „Kunde & Betrag" (bei Lieferscheinen seit Fix-Welle 1, S4, nur „Kunde" — ein Lieferschein trägt keinen Belegbetrag; mit `StatusBadge`; Kunde/Anschrift, Datum, Bruttobetrag — bei Rechnungen zusätzlich Bezahlt/Offen) und „Details" (ohne Badge; Fälligkeit/Gültigkeit, Zahlungs-/Lieferbedingungen, Steuerdaten, Netto/USt; bei Rechnungen seit Fix-Welle 1, S6, zusätzlich „Versendet am" mit Datum + Kanal aus dem jüngsten `EmailLog`-Eintrag, kein neues Feld — „Kanal" ist statisch „E-Mail", der einzige Versandweg). Die Zeilen bündeln weiterhin die vormals separaten „Empfänger"/„Eckdaten"-Karten (inkl. Kundenanschrift, bei Rechnungen zusätzlich `USt-IdNr.`, Fix-Welle M2) und reichen den jeweils wirksamen Status durch — bei Rechnungen `invoice.status` (bereits der wirksame Status aus `effectiveInvoiceStatus`, siehe `invoice-view-model.ts`), bei Dokumenten `effectiveQuoteStatus(...)` (page.tsx), bei Lieferscheinen `dn.status`. `InvoiceStatusCard` bettet in der „Details"-Karte den `PaymentDialog` sowie die mit „Zahlungen" beschriftete Zahlungsliste ein (Fix-Welle 1, S8; `id="zahlung"`-Anker aus `RowPaymentDialog`, siehe „Zahlungsdialog" unten, für den `#zahlung`-Primäraktions-Link im Kopf) — der Mahnblock (`PaymentSection`) sitzt seit Fix-Welle 1 (S7, Spec C „Detail-Layout") nicht mehr hier in der 24rem-Spalte, sondern volle Breite im `children`-Slot von `DocumentDetailLayout` (siehe „Rahmen" oben). Keine der drei Hüllen nimmt seit Task 5 noch ein `children`-Prop entgegen — `ShareLinkPanel`/`AttachmentPanel`/`DocumentChain`/`PrintOptionsPanel` reicht jede `page.tsx` stattdessen einzeln, seit Fix-Welle 1 (M1) ohne zusätzliche `DetailCard`-Hülle, an den `aside`-Slot von `DocumentDetailLayout` weiter (siehe „Kartenrahmen der rechten Spalte" oben).

**Zahlungsdialog** (`PaymentDialog`, `src/app/rechnungen/[id]/_parts/PaymentDialog.tsx`, Phase 13c, Task 3, überarbeitet Task 5 nach Review-Fund): kapselt keinen eigenen `<dialog>`-Rahmen mehr, sondern delegiert an `RowPaymentDialog` (`src/components/list/RowPaymentDialog.tsx`, ursprünglich Phase 13a für die Zeilen-Direktknöpfe in `RowActionsMenu`) — ein einziger nativer `<dialog>`-Rahmen für beide Einsatzorte. `RowPaymentDialog` ist ein kontrollierter Dialog (`open`/`onClose`-Props, `useEffect` ruft `showModal()`/`close()`); ein optionales `wide`-Prop schaltet über `style.maxWidth: "42rem"` auf dieselbe Breite wie Tailwinds `max-w-2xl` um, ohne die statische `className` zu verändern (die `test/unit/dialogs.test.ts`-Regel prüft `<dialog>`-Tags textuell auf `w-full`/`max-w-*` in `className="..."`). `PaymentDialog` selbst hält nur noch den offen/geschlossen-`useState`, den `id="zahlung"`-Anker sowie eine `close()`-Funktion, die **sowohl** `RowPaymentDialog`s `onClose` (Esc, ✕-Knopf) **als auch** `PaymentForm`s `onDone` (erfolgreiches Buchen) bedient und dabei den `#zahlung`-Hash aus der URL nimmt — sonst öffnet ein erneuter Klick auf denselben Bestandslink den Dialog nicht wieder, weil sich der Hash nicht ändert und `hashchange` nicht feuert.

**Statusknöpfe kompakt + „Mehr"-Menü** (`DocumentActions`/`DocumentActionsMenuItems`, `src/components/{DocumentActions,DocumentActionsMenu}.tsx`; `ActionMenu`/`ActionMenuItem`/`ActionMenuSeparator`, `src/components/detail/ActionMenu.tsx`): auf Dokument-/Lieferscheinseite zeigt die Kopfzeile seit der Fix-Welle (I2) nur noch `DocumentActions` `variant="compact"` — „Bearbeiten" (DRAFT) plus der erste verfügbare Statusübergang — statt aller Statusknöpfe in einer Reihe; die übrigen Übergänge sowie Archivieren/Aus Archiv/Duplizieren rendert `DocumentActionsMenuItems` als Menüzeilen im „Mehr"-Menü. Beide teilen sich die Netzwerk-/Dialoglogik über den Hook `useDocumentActions` (je Einsatzort eine eigene Instanz, da die beiden Stellen disjunkte Aktionsteilmengen zeigen und daher kein geteilter React-State nötig ist) und rendern bei Bedarf ihren eigenen Notiz-Dialog (Annehmen/Ablehnen). Das „Mehr"-Menü selbst ist ohne Client-JS über `<details>`/`<summary>` mit absolut positionierter Liste umgesetzt — funktioniert auch mit deaktiviertem JavaScript, Tastatur (Enter/Space auf `summary`) und Screenreadern ohne zusätzliche ARIA-Verdrahtung. Es bündelt neben `DocumentActionsMenuItems` weitere seltener genutzte, statusabhängige Aktionen: bei Rechnungen (`InvoiceMoreMenu`, unverändert `variant="default"`-frei, da `DocumentActions` dort gar nicht verwendet wird) XRechnung/ZUGFeRD-Export, Duplizieren, Teilgutschrift, Stornieren und die Lieferschein-Konvertierung (`ConvertMenu`); bei Dokumenten (`DocumentMoreMenu`, seit der Fix-Welle ohne eigenes `<ActionMenu>` — der Aufrufer kombiniert sie mit `DocumentActionsMenuItems` in einem gemeinsamen Menü) den `ConvertMenu` (Angebot→AB/Rechnung, AB→Rechnung, Teil-/Abschlags-/Schlussrechnung, →Lieferschein) plus einen „→ zur Rechnung"-Link, falls bereits konvertiert; bei Lieferscheinen inline im Detail-`page.tsx` die Teilrechnungs-Umwandlung sowie (nur `DRAFT`) einen Sprung zum Druckoptionen-Anker. Jeder Eintrag blendet sich weiterhin über dieselben `show*`-Bedingungen wie früher ein/aus — die eigentliche Prüfung bleibt serverseitig (409 bei Regelverstoß), das Menü ist nur die Darstellung. `ConvertMenu`/`DuplicateInvoiceButton` tragen seit M13 (Fix-Welle) ein `asMenuItem`-Prop für borderlose volle-Breite-Menüzeilen statt gerahmter Knöpfe, genutzt in `InvoiceMoreMenu`.

**Einklappbare Abschnitte** (`CollapsibleSection`, `src/components/detail/CollapsibleSection.tsx`): ebenfalls `<details>`-basiert, mit `summary`-Zusatztext (z. B. „12 Positionen · Netto 1.234,00 €") und optionalem `defaultOpen`. Trägt auf allen drei Detailseiten den Positionsblock. Die interne Notiz (`InternalNotesBox`, `src/components/detail/InternalNotesBox.tsx`) steht seit der Fix-Welle (I3) davor, außerhalb der Sektion, immer sichtbar — sie steht als einziges Beleg-Feld nirgends sonst (nie im PDF/XRechnung/ZUGFeRD/Mail).

**Sidebar-Hinweis** (`NavHint`, `src/components/shell/NavHint.tsx`, + `navHint`-State in `ShellProvider`): eine Detailseite ohne eigenen Listen-Pfad (z. B. `/dokumente/<id>` einer Auftragsbestätigung, die unter „Auftragsbestätigungen" in der Sidebar auftaucht, aber nicht unter der URL `/dokumente`) meldet der Sidebar per `useEffect` den Listen-Link, der stattdessen als aktiv gelten soll; `Sidebar.tsx` liest `navHint` statt `usePathname()`/`useSearchParams()`, sobald gesetzt. Der `useEffect`-Cleanup ruft `setNavHint(null)` beim Verlassen der Seite — kein Hint bleibt nach der Navigation hängen (nur `/dokumente/[id]/page.tsx` setzt ihn; Rechnung/Lieferschein haben einen eigenen Listen-Pfad und brauchen ihn nicht).

**Neighbor-Aware Query-Weitergabe**: alle drei Detailseiten hängen `?liste=<query>` an `prevHref`/`nextHref` weiter; der Zurück-Link trägt stattdessen direkt die bereits bereinigte `backQuery` als eigene Query (`/<Listenbasis>?<backQuery>`, kein `liste=`-Parameter) — in beiden Fällen bleibt „Vor/Zurück" so innerhalb derselben Liste, auch über mehrere Klicks hinweg.
...
3. **ZUGFeRD-Einbettung** — `src/lib/einvoice/zugferd.ts` reicht das CII-XML als `attachments`-Option an `renderInvoicePdf` durch; `doc.file(...)` (pdfkit-nativ, siehe unten) bettet es als Anhang (`factur-x.xml`, `AFRelationship: Alternative`) direkt beim Rendern ein. Der eingebettete XML-Teil bleibt führend (BMF 15.10.2025).
...
Damit entfällt der JVM-Sidecar vollständig.

### PDF/A-3b — eingebettete Schriften, `pdfkit`-Konformitätsmodus (Phase 14a, Task 6)

`pdfkit` 0.19 unterstützt PDF/A nativ (`subset: "PDF/A-3b"` erzeugt sRGB-`OutputIntent`, `pdfaid:part`/`conformance` in der XMP, `/AF`+`/AFRelationship` über `doc.file(...)`); die einzige verbleibende Hürde war die Schrifteinbettung — pdfkits eingebaute Helvetica ist eine AFM-Referenz ohne eingebettetes Font-Programm (verletzt ISO 19005-3, Klausel 6.2.11.4.1). Die Machbarkeit wurde vorab am 2026-09-12 mit veraPDF 1.30.2 geprüft (Spec R10, Scratchpad-Skript): mit eingebetteter Schrift **146 Regeln bestanden, 0 verletzt**; ohne eingebettete Schrift schlägt exakt diese eine Klausel fehl. **Am selben Tag erneut gegen den tatsächlichen Produktionscode bestätigt** (`scripts/render-pdfa-samples.ts` + veraPDF 1.30.2, lokal, Java 25): alle drei erzeugten Belege (Rechnung mit ZUGFeRD-Anhang, Lieferschein, Mahnung) `isCompliant="true"`, je **146 Regeln bestanden, 0 verletzt** (2101–2570 Einzelprüfungen je Datei). Der CI-Job (Task 7) führt denselben Nachweis automatisiert unter Java 17.

- **Fabrik** `src/lib/pdf/document.ts#createPdfDocument` ersetzt die drei bisherigen direkten `new PDFDocument(...)`-Aufrufe (`invoice-pdf.ts`, `delivery-note-pdf.ts`, `dunning-pdf.ts`). Sie setzt `pdfVersion: "1.7"`, `subset: "PDF/A-3b"` und **zwingend** `font: null`, ruft danach `registerPdfFonts(doc)` (siehe unten) und anschließend einmal `doc.font("Helvetica")`.
  `font: null` ist kein Stilmittel: ohne diese Option ruft der `PDFDocument`-Konstruktor selbst `this.font("Helvetica")` auf, **bevor** die Anwendung `registerFont` überhaupt aufrufen kann — pdfkit cached die eingebaute AFM-Helvetica dabei bereits unter dem Namen `"Helvetica"`. Ein späteres `registerFont("Helvetica", <ttf>)` ändert an diesem Cache-Eintrag nichts mehr, `doc.font("Helvetica")` liefert weiterhin die nicht eingebettete Standardschrift — praktisch nachgewiesen (Datei blieb non-compliant). Mit `font: null` bleibt der Font-Slot leer, bis die Fabrik **nach** `registerFont` selbst `doc.font("Helvetica")` aufruft (nötig, weil nicht jede Zeichenstelle im Bestand vor ihrem ersten `doc.text(...)` selbst `.font(...)` setzt, z. B. `drawSenderLine`) — ab da zeigt „Helvetica" korrekt auf die eingebettete Schrift.
- **Schriften** `src/lib/pdf/fonts.ts#registerPdfFonts` registriert vier TTF-Dateien unter den **bestehenden** Namen `"Helvetica"`/`"Helvetica-Bold"`/`"Helvetica-Oblique"`/`"Helvetica-BoldOblique"` — keiner der ca. 32 `doc.font(...)`-Aufrufe im Bestand wird angefasst. Die Dateien liegen unter `assets/fonts/` und werden einmalig gelesen und als Buffer gecached.
  **Herkunft:** Liberation Sans (Copyright Red Hat, Inc., 2012; SIL Open Font License 1.1, Volltext in `assets/fonts/LICENSE-OFL.txt`) — metrisch kompatibel zu Arial/Helvetica (gemessene Abweichung auf einer 247-pt-Zeile: 0,73 pt / 0,3 %, kein Layoutumbruch). Der offizielle GitHub-Release (`liberationfonts/liberation-fonts`) liefert nur den Quellcode (Build über FontForge aus `.sfd`-Dateien); die vier fertigen TTFs stammen unverändert aus dem `standard_fonts/`-Bundle des npm-Pakets `pdfjs-dist` (Mozilla PDF.js), das exakt dieselben, unmodifizierten Liberation-Sans-Originaldateien mitliefert — Familienname/PostScript-Name intern verifiziert (`fontkit`: `Liberation Sans` / `LiberationSans{,-Bold,-Italic,-BoldItalic}`), volle Glyphenabdeckung für deutsche Umlaute, ß, €, § und Anführungszeichen bestätigt.
- **ZUGFeRD-Anhang** setzt `doc.file(bytes, { name, relationship, type, description })` **während** des Renderns in `invoice-pdf.ts` (statt wie bisher per `pdf-lib` im Nachgang); `pdf-lib` war danach ohne weiteren Nutzungsort und wurde entfernt. `src/lib/pdf/pdfa-xmp.ts#buildFacturXXmp` hängt zusätzlich das Factur-X-`pdfaExtension:schemas`-Erweiterungsschema (`fx:DocumentType`, `fx:DocumentFileName`, `fx:Version`, `fx:ConformanceLevel`) per `doc.appendXML(...)` an — nur im ZUGFeRD-Pfad, Rechnung/Lieferschein/Mahnung ohne Anhang bleiben unverändert.
- **Typ-Lücken in `@types/pdfkit`:** `PDFDocumentOptions.font` kennt nur `string | undefined` (kein `null`), `PDFKit.Mixins.PDFAttachmentOptions` kennt kein `relationship` — beide Male zur Laufzeit aber ausdrücklich unterstützt (siehe `node_modules/pdfkit/js/pdfkit.js`, `FontsMixin.initFonts`/`AttachmentsMixin.file`). Statt `any` je ein gezielter, kommentierter Interop-Fix: `document.ts` castet über `as unknown as PDFKit.PDFDocumentOptions`, `invoice-pdf.ts` erweitert `PDFAttachmentOptions` strukturell um `relationship` (Intersection-Typ, kein Cast nötig).
- **Nachweis:** `test/unit/pdfa-structure.test.ts` (Java-frei, reine Bytesuche — pdfkit schreibt Objekt-Dictionaries und die XMP-Metadatenstream immer unkomprimiert, `metadataRef.compress = false`) plus veraPDF in CI (Task 7).

### PDF/A-Nachweis in CI und Bildabsicherung (Phase 14a, Task 7)

- **Muster-Generator** `scripts/render-pdfa-samples.ts` rendert drei Belege (ZUGFeRD-Rechnung mit Logo + GiroCode, Lieferschein, Mahnung) nach `tmp/pdfa-samples/` — reine Musterdaten (`src/domain/settings/preview.ts#buildSample*`), **kein DB-Zugriff**, ein selbst erzeugtes RGB-PNG als Logo (kein Fixture-Binary). `npx tsx scripts/render-pdfa-samples.ts` funktioniert auch lokal.
- **CI** (`.github/workflows/ci.yml`, Job `xrechnung-kosit`, Java 17 liegt für den KoSIT-Schritt bereits vor): nach der KoSIT-Validierung installiert ein Schritt veraPDF headless über den offiziellen Installer (`software.verapdf.org/releases/verapdf-installer.zip`, IzPack-Automatikdatei nach dem offiziellen `auto-install-tmp.xml`-Muster des `veraPDF-apps`-Repos, nur das CLI-Paket ausgewählt — die drei Pakete GUI/CLI/Doku sind laut `installer/src/main/izpack/install.xml` unabhängig voneinander), ein zweiter Schritt prüft `verapdf -f 3b tmp/pdfa-samples/*.pdf`. Exit-Code 1 bei mindestens einer nicht konformen Datei, 0 wenn alle konform sind (`CliConstants.ExitCodes`) — kein zusätzliches Report-Parsing nötig, ein fehlerhafter Lauf lässt den CI-Schritt direkt fehlschlagen. Der Anwendungscontainer bleibt unberührt (kein Java im `Dockerfile`).
  **Ruling (Java-17-Ausweiche nicht gezogen):** lokal am 2026-09-12 verifiziert (Installer + `verapdf -f 3b` gegen alle drei erzeugten Muster, siehe oben) — unter Java 25 (Homebrew, kein Java 17 im Sandbox-Image verfügbar, langsame Download-Strecke zu `software.verapdf.org` verhinderte den `openjdk@17`-Bau innerhalb der Sitzung). Laut offizieller veraPDF-Dokumentation (`veraPDF-apps` README) wird die Installation unter Java 8, 11, 17 **und** 21 unterstützt; ein Erfolg unter der neueren Version 25 ist ein starkes, aber kein 1:1-Indiz für Java 17 im CI-Runner. Die im Plan vorgesehene Ausweichregel (eigener Job mit Java 11, falls der Schritt unter Java 17 abbricht) ist deshalb **nicht** präventiv gezogen, sondern bleibt Beobachtungspunkt für den ersten echten CI-Lauf dieses Branches — bricht er dort ab, ist das Java-11-Fallback nachzutragen, nicht stillschweigend wegzulassen.
- **Bildabsicherung** `src/lib/images/jpeg-info.ts#jpegComponents` liest ausschließlich den ersten SOFn-Marker (kein Bildpaket, Muster `png-size.ts`) und liefert die Komponentenzahl (3 = RGB/YCbCr, 4 = CMYK/YCCK). Der Logo-/Hintergrund-Upload (`src/app/api/settings/branding/upload/route.ts`) lehnt ein JPEG mit 4 Komponenten für `kind=logo`/`background` mit 400 ab — diese beiden werden ins PDF/A eingebettet, `favicon`/`applogo` nicht. Ein CMYK-JPEG erzeugt in pdfkit `DeviceCMYK` ohne passenden OutputIntent und würde die Konformität sonst wortlos brechen.
  **Bestandsschutz statt stillem Verschwinden:** ein bereits vor Task 7 hochgeladenes CMYK-Logo/-Hintergrundbild wird weiterhin gedruckt (kein Datenverlust); `src/app/einstellungen/briefpapier/page.tsx` liest die hinterlegte Datei serverseitig, prüft sie mit `jpegComponents` und reicht das Ergebnis als `cmykWarning`-Prop an `BrandingForm` — ein Hinweis über der jeweiligen „Aktuell: …"-Zeile, kein Upload-Zwang.
...
src/
  ...
  components/             # UI-Komponenten, inkl. forms/ (CustomerForm.tsx, OrganizationForm.tsx,
                           ...
                           # charts/               # Inline-SVG-Baukasten (Phase 12e): BarChart.tsx,
                           # LineChart.tsx, DonutChart.tsx, ChartFrame.tsx, types.ts — keine
                           # Chart-Bibliothek, keine "use client"-Komponenten
                           # dashboard/             # chart-data.ts (Aufbereitung fuer den Baukasten),
                           # DashboardWidgets.tsx, AgingChart.tsx (Phase 12e)
  ...
  domain/                 # framework-frei, testbar
    ...
    reporting/               # revenue.ts (monthlyRevenue/netShareCents), customers.ts
                           # (topCustomers), status.ts (statusCounts), payment-behaviour.ts
                           # (paymentBehaviour), query.ts (runReport/reportQuerySchema —
                           # gemeinsamer Kern fuer GET /api/v1/Report + MCP get_report, Phase 12e)
