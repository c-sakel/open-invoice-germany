...
**Umsatz je Monat und Top-Kunden** (`monthlyRevenue`/`netShareCents`/`monthKey`, `src/domain/reporting/revenue.ts`; `topCustomers`, `src/domain/reporting/customers.ts`): reine, org-gescopte Aggregationsfunktionen über `select`-reduzierte `Invoice`-Zeilen + JS-Summierung (dieselbe DB-Portabilitäts-Begründung wie `dashboardSummary`, siehe Abschnitt „Workflow & UX … Dashboard" oben). Basis ist **Netto**, nicht Brutto — `netShareCents(inv)` liefert ohne `payableCents` schlicht `netTotalCents`; trägt die Rechnung `payableCents` (Schlussrechnung einer Abschlagskette, §14), skaliert sie `netTotalCents` proportional zum Verhältnis `payableCents / grossTotalCents` (`roundHalfUp`). Ohne diese Skalierung würde eine Abschlags- plus Schlussrechnung den Abschlag doppelt zählen — derselbe Fehler, den die Fix-Welle S2 für `revenueThisMonthCents`/`grossTotalCents` in `dashboardSummary` behoben hat (dort mit `payableBaseCents`, brutto, für die aktuelle Monatskachel; hier mit `netShareCents`, netto, für die 12-Monats-Reihe/Kundenrangliste — zwei unterschiedliche Bemessungsgrundlagen für zwei unterschiedliche Kennzahlen, keine Doppelimplementierung derselben Sache, Lastenheft 1.4). `monthlyRevenue` liefert eine lückenlose 12-Monats-Reihe (Default `months`, überschreibbar, optionaler `customerId`-Filter), `topCustomers` gruppiert dieselbe Beleg-Menge je `customerId`, sortiert absteigend nach Netto-Umsatz und kappt bei `limit` (Default 5; die Validierung ungültiger Werte übernimmt die Boundary — API-Route/MCP-Tool, Task 5 —, nicht die Domainfunktion selbst).

**Vorzeichen und Storno — periodengerechte (accrual) Sicht** (Fix-Welle 1, Review-Befund): beide Funktionen summieren `netShareCents` **unverändert**, ohne ein zusätzliches `type === "CREDIT_NOTE" ? -1 : 1`. Gutschriften/Stornos, die über die bestehenden Domain-Funktionen entstehen (`createPartialCreditNote`, `src/domain/invoice/credit.ts`; `cancelInvoice`, `src/domain/invoice/cancel.ts`), tragen bereits **negative** `netTotalCents`/`grossTotalCents` (betragsspiegelbildlich zum Original) — ein zusätzlicher Vorzeichendreh hätte eine echte Gutschrift ein zweites Mal negiert und den ausgewiesenen Umsatz dadurch erhöht statt gemindert. Der Statusfilter schließt nur `status === "DRAFT"` aus, **nicht** `CANCELLED`: das stornierte Original bleibt unverändert (GoBD-Unveränderbarkeit) und zählt weiterhin mit vollem Betrag in seinem Ausstellungsmonat; die Storno-Gutschrift selbst trägt ihr eigenes `issueDate` (Stornozeitpunkt) und mindert dort separat. Fallen Original und Storno in unterschiedliche Monate, zeigt die Reihe also `+X` im Ausstellungs- und `-X` im Stornierungsmonat — über den vollen Zeitraum gleicht sich das exakt aus (Summe 0), statt eine einzelne Periode fälschlich negativ auszuweisen (Test: `test/integration/reporting-revenue.test.ts`, Storno-Szenario Februar/März).
