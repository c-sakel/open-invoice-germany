...
**Statuskarte** (`StatusCard`, `src/components/detail/StatusCard.tsx`): generische Karte mit Titelzeile (`status`-ReactNode rechts daneben, i. d. R. `StatusBadge`), einer `dt`/`dd`-Zeilenliste (`StatusRow[]`) sowie `children` darunter für interaktive Blöcke. Je Belegtyp gibt es eine eigene, typisierte Hülle: `InvoiceStatusCard`/`DocumentStatusCard`/`DeliveryNoteStatusCard` (`src/app/{rechnungen,dokumente,lieferscheine}/[id]/_parts/`) bündeln die vormals separaten „Empfänger"/„Eckdaten"-Karten als kompakte Zeilen und reichen den jeweils wirksamen Status als `<StatusBadge status={…} />` durch — bei Rechnungen `invoice.status` (bereits der wirksame Status aus `effectiveInvoiceStatus`, siehe `invoice-view-model.ts`), bei Dokumenten `effectiveQuoteStatus(...)` (page.tsx), bei Lieferscheinen `dn.status`. `InvoiceStatusCard` bettet zusätzlich `PaymentForm`/Zahlungsliste/Mahnblock ein (`id="zahlung"`-Anker für den `#zahlung`-Primäraktions-Link im Kopf); `DocumentStatusCard`/`DeliveryNoteStatusCard` nehmen `ShareLinkPanel`/`AttachmentPanel`/`DocumentChain` als `children` entgegen.

**„Mehr"-Menü** (`ActionMenu`/`ActionMenuItem`/`ActionMenuSeparator`, `src/components/detail/ActionMenu.tsx`): ohne Client-JS über `<details>`/`<summary>` mit absolut positionierter Liste — funktioniert auch mit deaktiviertem JavaScript, Tastatur (Enter/Space auf `summary`) und Screenreadern ohne zusätzliche ARIA-Verdrahtung. Bündelt seltener genutzte, statusabhängige Aktionen, die vorher in der Kopfzeile bzw. in eigenen Korrekturbereichen standen: bei Rechnungen (`InvoiceMoreMenu`) XRechnung/ZUGFeRD-Export, Duplizieren, Teilgutschrift, Stornieren und die Lieferschein-Konvertierung (`ConvertMenu`); bei Dokumenten (`DocumentMoreMenu`) den `ConvertMenu` (Angebot→AB/Rechnung, AB→Rechnung, Teil-/Abschlags-/Schlussrechnung, →Lieferschein) plus einen „→ zur Rechnung"-Link, falls bereits konvertiert; bei Lieferscheinen inline im Detail-`page.tsx` die Teilrechnungs-Umwandlung sowie (nur `DRAFT`) einen Sprung zum Druckoptionen-Anker. Jeder Eintrag blendet sich weiterhin über dieselben `show*`-Bedingungen wie früher ein/aus — die eigentliche Prüfung bleibt serverseitig (409 bei Regelverstoß), das Menü ist nur die Darstellung. Fehlt jede Bedingung, rendert die jeweilige `*MoreMenu`-Komponente `null` (kein leeres Menü).

**Einklappbare Abschnitte** (`CollapsibleSection`, `src/components/detail/CollapsibleSection.tsx`): ebenfalls `<details>`-basiert, mit `summary`-Zusatztext (z. B. „12 Positionen · Netto 1.234,00 €") und optionalem `defaultOpen`. Trägt auf allen drei Detailseiten den Positionsblock; bei Rechnungen zusätzlich „Zahlung erfassen" (`defaultOpen={canPay}`, in `InvoiceStatusCard`).
...
**Neighbor-Aware Query-Weitergabe**: alle drei Detailseiten hängen `?liste=<query>` an `prevHref`/`nextHref`/Zurück-Link weiter, damit „Vor/Zurück" innerhalb derselben Liste bleibt, auch über mehrere Klicks hinweg.
