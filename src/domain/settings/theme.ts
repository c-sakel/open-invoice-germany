 ...
 * printOptionsJson`) und die Logo-/Hintergrundbild-Buffer.
 */
export async function loadPdfTheme(orgId: string, overrideJson?: string | null): Promise<PdfTheme> {
  const [brand, printSettings] = await Promise.all([loadBrandingSettings(orgId), loadPrintSettings(orgId)]);
  ...
  return { brand, options, logoBuffer, backgroundBuffer };
}
