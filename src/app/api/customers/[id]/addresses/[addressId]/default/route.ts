...
export async function POST(_req: Request, ctx: { params: Promise<{ id: string; addressId: string }> }) {
  const { id, addressId } = await ctx.params;
  try {
    const org = await getActiveOrg();
    const address = await setDefaultAddress(org.id, id, addressId);
    return NextResponse.json({ address });
  } catch (e) {
    return mapError(e, "POST /api/customers/[id]/addresses/[addressId]/default:");
  }
}
