 ...
 * `Idempotency-Key` eine gespeicherte Antwort ausliefern oder nach dem Handler
 * speichern -> Fehler einheitlich mappen (src/api/errors.ts). `/api/v1/*` akzeptiert
  ...
  return async (req: Request, routeCtx?: ApiRouteContext<TParams>): Promise<NextResponse> => {
    try {
      ...
      if (usesIdempotency) {
        ...
        const replay = await checkIdempotency(ctx.orgId, idemKey!, method, url.pathname, rawBody);
        if (replay) {
      ...
      if (usesIdempotency && res.status < 500) {
        const replayBody = await res.clone().json().catch(() => null);
        await storeIdempotentResponse(ctx.orgId, idemKey!, method, url.pathname, rawBody, res.status, replayBody);
      }
